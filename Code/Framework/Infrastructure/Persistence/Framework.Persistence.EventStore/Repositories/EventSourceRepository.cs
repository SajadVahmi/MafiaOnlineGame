...
namespace Framework.Persistence.EventStore.Repositories
{
        ...
        where TKey : notnull
    {
        public async Task<T> GetByIdAsync(TKey id, CancellationToken cancellationToken = default)
        {
            ...
            var listOfEvents = await eventStore.GetEventsOfStreamAsync<T,TKey>(id, snapshot.Version, jsonSerializerAdapter);
        ...
        {
            await eventStore.AppendEventsAsync<T,TKey>(aggregate, jsonSerializerAdapter);
